...
### ENV-FR-010 生成物のトラッキングと env-destroy による撤去
...
- **説明**: フック（即効層）はプラグイン無効化で消えるが、env-init が生成した恒久層
  ...
  env-destroy スキルがそれを根拠にユーザー確認付きで安全に撤去できなければならない。
...
- **受入基準 (EARS)**:
  ...
  - WHEN env-destroy を実行する THEN システムはレジストリの記録に基づき撤去対象の一覧を提示し、ユーザー確認を得てから撤去する SHALL
