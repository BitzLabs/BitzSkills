...
**要件化・実装完了（2026-07-30）**: 提案1〜4 を `SDD-FR-163` として起票し、
`SDD-TSK-052` で実装・検証した（証跡 `.spec/verification/pytest--0ee41c9.json`）。

- **提案1**（`done` の不変条件）— **実装済**（`SDD-FR-163`）。「`done` は未完了成果物が
  存在しないときだけ返す」を定義し、`draft` 要件が1件以上あれば `done` を抑止する。
- **提案2**（修正方式の選定）— **実装済**（`SDD-FR-163`）。既存 `phase_code` を保ったままの
  加算（候補 (c) 相当）を採用した。
  `phase_code` の既存値は削除・改名しない（`SDD-FR-136` の公開契約を維持）。
  feature・世代単位の集計（候補 (b)）は V4 の Workspace 責任モデルと接する論点であり、
  いま構造を増やさない。
- **提案3**（`phase_code` と `next_actions` の整合条件）— **実装済**（`SDD-FR-163`）。
- **提案4**（完了済みベースラインへ `draft` を追加する回帰テスト）— **実装済**。回帰テスト5件。
- 残余リスク: 公開契約（語彙）は維持するが、**同じワークスペースが返す `phase_code` の値は
  変わる**。棚卸しの結果、条件分岐は `spec_status.py` 自身の `next_actions`（`design` /
  `plan` / `verify` の3箇所）だけで、他スキルは表示用の `phase` を読むのみだった。
  `plan` 分岐は `n_tasks == 0` を併せて要求するため完了済み系列では発火せず、影響はない。
