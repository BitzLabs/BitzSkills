...
**要件化・実装完了（2026-07-30）**: 提案1〜3 を `SDD-FR-162` として起票し、
`SDD-TSK-051` で実装・検証した（証跡 `.spec/verification/pytest--5526358.json`）。

- **提案1**（`design/**/*.md` の再帰走査）— **実装済**（`SDD-FR-162`）。裁定時の実測で、再帰化により新たに
...
- **提案2**（重複時に両方のパスを示す）— **実装済**（`SDD-FR-162`）。
- **提案3**（frontmatter の `id:` を根拠にした採番）— **実装済**（`SDD-FR-162`）。
  `SI-SDD-006` 提案2 の再提案を今回は見送らなかった。`id:` を持たない成果物は
  ファイル名抽出へフォールバックし、説明的サフィックスの慣行（同 spec-issue 提案1）は維持した。
