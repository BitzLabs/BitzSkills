...
- **背景 / 課題**:
  ...
  - **`review-synthesis.json` は最新1件で上書きされる**。SDD-REV-002〜005 は番号付き
    `.json` として保存されているが、**SDD-REV-006 は `SDD-REV-006.json` として保存されていない**
    （実測で確認）。次のレビューを実行した時点で、SDD-REV-006 の findings 10件と
    `gate_preconditions` 5件は失われる。実害が今まさに発生しかけている。
...
- **実装順序**:
  1. `SDD-REV-006.json` のアーカイブ（**先に実データを救う**。schema 変更を待たない）
  2. `synthesis.md` の schema 固定と `sdd-review` SKILL.md の更新
  3. `spec_inspect` の検査3種を追加（アーカイブ漏れ → `tracked_by` 実在 → 未紐づけ P0/P1）
  4. `carried_over[]` の生成手順を `synthesis.md` に規定

  1 は他の裁定にも依存せず即実施できる。**先に消えかけているデータを保全する**。
