...
## 未解決（Design Gate で人間が裁定する）

- (1) 判定機能をどちらのコンテキストへ寄せるか — SI-SDD-030 の前提
- (2) `GatePassage` を導入するか — SI-SDD-028 の前提
- (3) `ReviewFinding` の独立と `tracked_by` 必須化 — SI-SDD-031 の前提
- (4) 集約分割に伴う遷移ポリシーの再構成（破壊的変更を伴う可能性）
- (5) `manual-check` の扱い（語彙から外す / 証跡へ格上げする）— SI-SDD-029 の前提
- (6) sdd-usecase の配置 — SI-SDD-013 の前提
- **bitz-flow との境界（sdd-git 移管）は本モデルでは確定させない**（人間裁定・保留）
