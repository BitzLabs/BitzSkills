...
- **発動テスト（縮小版・実測）**: 発動判定9ケース（境界ケース T3「どこまで進んだ？」T4「進捗を教えて」を含む）を
  新セッションの判定者2体で独立実行（提示順シャッフル・期待解は伏せる）。**18/18 期待どおり**
  （sdd-plan 発動3・sdd-report 維持2・対照2 すべて正答）。記録: `evals/sdd-plan/`（cases.md, runs/1〜2） ✅
  （あり/なし比較のフル skill-tester は 2026-07-16 人間裁定で省略 — 既存スクリプトの案内層のため効果測定は過剰）
- **検証ステータス**: green（2026-07-16。目視・機械チェック・発動テストすべて PASS）
...
- **発動テスト（縮小版・実測）**: sdd-plan と合同実施（`evals/sdd-issue/cases.md` → 実体は evals/sdd-plan/）。
  sdd-issue 発動2ケース（T6/T7）と sdd-core に譲るべき境界ケース（T8 要件の deprecated 化）を含む
  9ケース×判定者2体で **18/18 期待どおり**。誤爆・取りこぼしなし ✅
  （あり/なし比較のフル skill-tester は 2026-07-16 人間裁定で省略）
- **検証ステータス**: green（2026-07-16。目視・機械チェック・発動テストすべて PASS）
