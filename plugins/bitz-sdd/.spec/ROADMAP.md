...
**2026-07-30 に順序を見直した**（`decision-2026-07-30-implementation-order.md` 裁定C）。
Design Gate は SI-CORE-038 を最先行に置いたが、その根拠が実測で成り立たなかった（SI-SDD-035）。
他の裁定に依存しない加法的な変更を先頭に置き、SI-CORE-038 は裁定1の直前へ縮小して移した。

**同日さらに順序7 を再縮小した**（`decision-2026-07-30-order7-scope.md` 裁定E）。
`REL` 一般化が必要になるのは「裁定1 で sdd-test 側に立つツールをラッパー経由で公開する」場合に
限られるが、その移設の形は未設計（`SI-SDD-030` は未着手）であり **`basis` は assumed** である。
`SDD-FR-161` で機械検証まで入れた不変条件（`assumed` を根拠に `blocking` を立てない）を
自分自身へ適用し、`REL` 一般化は必要性が `verified` になる順序8 へ移した。

| 順 | 内容 | 性質 |
|---|---|---|
| 6 | **裁定2 GatePassage** / **裁定3 ReviewFinding**（並行可） | 加法的。他の裁定に依存しない |
| 7 | **SI-CORE-038** 呼び出し規約の明文化（AGENTS.md ＋ `CORE-CON-*` で機械検証） | 加法的。**再縮小済み** |
| 8 | **裁定1 + 裁定5** 証跡 schema 拡張と判定移設 ＋ `REL` のツールごと一般化 | 破壊的（証跡 schema）。**ここで 4.0.0 へ bump** |
| 9 | **裁定4** 集約分割 | 破壊的（CLI 契約） |
| 10 | **SI-SDD-032** mtime 精度・mutation lock 不参加 | 独立して実施可。裁定待ち |
...
- **破壊的変更は許容**（人間裁定 2026-07-29）。ただし公開契約を壊す場合は、
  ...
  波及を移行計画として明示する
- **メジャー bump は順序8 まで停止する**（人間裁定 2026-07-30、裁定A）。marketplace は
  `"source": "./plugins/bitz-sdd"` でバージョンを固定しないため main へのマージが即配布になる。
  加法的な順序6・7 は 3.x の minor/patch で進め、**最初の破壊的変更（順序8）に着手する時点で
  4.0.0 へ一括 bump** する。3.x に破壊的変更を混ぜない
