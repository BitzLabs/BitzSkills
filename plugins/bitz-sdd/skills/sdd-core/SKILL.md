...
docs/                        永続・人間ナラティブ（.spec から自動生成 / 逆同期対象）
  MASTER.md / 00_はじめに/ / 01_システム仕様/ / 02_ユースケース/ / 03_設計仕様/ / 04_テスト仕様/ / 05_リリース・運用/
.spec/                       仕様・設計・検証のマスター
...
`done` は**未完了成果物が無いときだけ**返る — 完了済み系列へ次期版の `draft` 要件を足した
状態は `plan` であり、Promotion Gate 待ちではない（SDD-FR-163）：
