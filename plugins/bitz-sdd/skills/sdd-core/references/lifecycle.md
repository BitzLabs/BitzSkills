...
### accepted spec-issue の完了判定 — 3分類と2つの警告（SDD-FR-141 / SDD-FR-142）

`spec_status.py` は accepted spec-issue を次の3つに分類する:

1. **対応済み**（非表示）— 本文に `**実施**:` マーカーがある、または**実行スコープ内**の
   いずれかのワークスペースの要件 `origin:` がその ID を参照する
2. **委譲済み・未解決**（`accepted_delegated_unresolved`）— frontmatter に `delegated_to:` を持つが
   1 に該当しない。クロス WS 委譲済みだが、実行スコープに委譲先ワークスペースが含まれず
   `origin:` を辿れない場合に該当する（**単一スコープ実行の false-positive を防ぐため**、従来の
   `accepted_unaddressed` からは分離する。SDD-FR-141）。`--workspace` 一括実行で委譲先を含めれば
   1 に解消される
3. **未着手**（`accepted_unaddressed`）— 上記いずれにも該当しない（従来どおり）

あわせて、1 のうち**参照元要件が verified/promoted かつ本文に `**実施**:` マーカーが無い**ものを
`completion_record_missing` として警告する（SDD-FR-142）。判定・件数は 1〜3 に影響しない
読み取り専用の督促で、対象 issue に実施マーカーを追記すれば解消する。origin 参照は「未着手ではない」と
正しく判定できても issue 単体から実装根拠を辿れないため、記録の追記を促す。

## ワークスペース間の spec-issue 委託（SDD-FR-132）

モノリポ等でルート SPEC とサブ（ワークスペース）SPEC が併存する場合、spec-issue の
委託は次の2方向のみを正式なフローとする:

- **サブ→ルート（エスカレーション）**: 共通規約・複数ワークスペースに跨る問題は
  ルートへエスカレーションする
- **ルート→サブ（委任）**: 単一ワークスペースに閉じる問題はサブへ委任する
- **サブ↔サブ（禁止）**: サブ間の直接委託は行わず、**常にルート経由**とする。
  複数サブに影響する問題はルートで管理し、ルートから各サブへ委任を出す
  （裁定 2026-07-19: 依存宣言済みペアへの直接委託例外は不採用。委託グラフを
  ルートを頂点とする星型に保ち、追跡と検証を単純にするため）
- **ルート SPEC 不在のリポジトリ**: ルート `.spec/` を作成してから委託する
  （既存のルート作成フローどおり）

### frontmatter 書式

委託元 spec-issue に記録する（委託が生じた場合のみ。既存 spec-issue への後付けは不要）:

```yaml
origin: root                                  # 起票ワークスペース（注記の併記可）
delegated_to: bitz-sdd:SDD-FR-132             # 委託先。`<ws>:<ID>` のカンマ区切り
```

- `delegated_to:` の ID は委託先ワークスペースの**要件・spec-issue・タスクのいずれか**。
  ルート→サブの委任は委任先要件（reverse-derived 起票を含む）を指すのが通例
  （SI-CORE-031/032 の先例）
- 受け手側は自身の `origin:` に委託元 spec-issue ID を言及して**双方向リンク**にする
  （要件なら `origin: SI-CORE-031`、spec-issue なら `origin: root（SI-CORE-032 の
  実装振り返り）` のような注記付き自由記述でもよい — 検証は ID の「言及」で突合する）

### 機械検証（spec_inspect.py）

`--workspace` 一括起動時に全ワークスペース横断で検証する:

- `delegated_to:` の各エントリの ID 部が既知 ID（要件∪spec-issue∪タスク）に実在すること
  （リンク切れは FAIL）
- 委託先 frontmatter の `origin:` に委託元 spec-issue ID への言及があること
  （双方向リンク欠如は FAIL）
- `<ws>:` 修飾部は人間向け情報で検証対象外。`origin:` / `delegated_to:` を持たない
  spec-issue はチェック対象外（後方互換）
...
python scripts/spec_inspect.py <repo-root> --impact-docs docs/03_設計仕様/アーキテクチャ.md
```
