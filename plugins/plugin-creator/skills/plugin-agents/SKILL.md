...
`agents/*.md` は Claude Code の形式だが、Antigravity 2.0 の agy CLI も
同形式をサブエージェントとして取り込む（Claude 互換レイヤー）。
両対応プラグインでは `model: inherit` を使い、`tools` のツール名が
Claude Code 固有である点に留意する（Antigravity 側では解釈されない
可能性があるため、tools 制限に安全性を依存させない）。
...
**ベストプラクティス**: 具体例を2〜4個。proactive（能動的）と受動的
...
`sonnet` / `opus` / `haiku` は Claude Code 固有のモデル名なので、
**Antigravity 2.0 でも使うプラグインでは必ず `inherit` にする**
（Antigravity は Gemini 系モデルで動くため）。
