...
本文は Claude Code のマニフェスト仕様。Antigravity 2.0 との違いは
末尾の「Antigravity 2.0 のマニフェスト」を参照。
...
## Antigravity 2.0 のマニフェスト

Antigravity は**プラグインルート直下**の `plugin.json` を読む
（`.claude-plugin/plugin.json` は読まれない）:

```json
{ "name": "team-developer-kit" }
```

- **必須フィールドはない**。ファイルの存在自体がプラグインの宣言で、
  `name` 省略時はフォルダ名が使われる
- `commands` / `agents` / `hooks` / `mcpServers` などのコンポーネントパス
  フィールドは解釈されない。フックは `hooks.json`、MCP は `mcp_config.json`
  を**ルート直下の固定パス**に置く
- コマンドラインでの検証: `agy plugin validate <path>`

### 両対応プラグインのマニフェスト運用

1. `.claude-plugin/plugin.json`（Claude Code 用）とルートの `plugin.json`
   （Antigravity 用）の両方を置く
2. `name` / `description` / `version` は両ファイルで同じ値に保つ
   （特に `version` の同期を release チェックリストに入れる）
3. Claude Code 専用のコンポーネントパスフィールドは
   `.claude-plugin/plugin.json` 側にのみ書く
