...
> **訂正（2026-08-15、`investigation-2026-08-15-capability-reduction.md`）**
>
> 当初ここには「つまり**外部の鍵保持者は要件上存在しない**」と書いたが、**これは言い過ぎだった**。
> `FLW-DSN-016` の脅威モデル節（L729-731）は「秘密鍵は可能なら executor と別の
> owner-only process/keystore へ隔離する。**隔離不能なら悪意ある executor への防御を主張せず、
> 単回 nonce・監査 chain・明示承認による事故防止だけを保証する**」と明記しており、
> 設計は自ら claim を縮退させている。過大な主張はしていない。
>
> また `RSK-204` の含意も過大だった。`apply()` 自体は registry を読まないが、
> **公開経路は `cli.py:405` が `load_trusted_keys()` を呼んで強制している**。
> 鍵を直接注入しているのは library 直呼びのテストである。
>
> したがって B の争点は「設計が過大な主張をしているか」ではなく、
> **「鍵を隔離できない配備において、この機構は割に合うか」**である。
> 選択肢と推奨は上記 investigation を参照。

現状の配備では、`load_trusted_keys()` が読む registry が被験リポジトリ自身の
git common-dir 配下にあるため、鍵は executor と同じ信頼領域にある。
署名者と検証者が同じ領域にある限り、署名は宣言された3脅威のいずれにも直接対応しない
（対応しているのは nonce 単回性と scope / freshness の比較である）。
§3.2 の再発3類型のうち「実体のない層を維持するコスト」は、この点に由来する可能性がある。
