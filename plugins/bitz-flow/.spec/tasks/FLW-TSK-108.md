...
boundary: plugins/bitz-flow/skills/flow-core/scripts/flowlib/worktree_lease.py,tests/test_flow_m2_process_lease.py
status: pending
---
...
- **完了条件**: 複数process競合でmutationへ進むprocessが最大1つとなり、各crash pointからの再開が
  設計表どおり収束する。child終了を証明できない場合は後続mutationを停止する。
