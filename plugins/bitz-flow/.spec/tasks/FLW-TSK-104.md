...
- **完了判定（2026-08-22）**: 競合applyが先行operationのmutation中に`BLOCKED`となり、
  先行完了後にheld targetが空になる対照テストを確認した。`tests/test_flow_m2_runtime.py`を
  含む対象3スイートは合計`143 passed`（終了コード0）。process間leaseとcrash後quarantineは
  当初の境界どおり後続タスクへ残す。
