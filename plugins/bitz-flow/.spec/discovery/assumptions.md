...
| H-F2 | Feasibility | Python 3.10+標準ライブラリで必要なGit / gh出力を安全にparseできる | クリティカル | 部分支持 | status/diff/log/Issue/PR/releaseのgolden fixture。必須field保持100%未満なら対象操作を縮小し、解決不能ならNo-Go。別言語へ移行しない |
...
4. **H-F2**: Python 3.10+で安全性・可搬性を成立させられなければ、scope縮小またはNo-Goとする。
...
- **条件**:
  ...
  6. 実装言語はPython 3.10+に固定し、Goへの移行・再実装・比較検討を行わない。
- **裁定者**: hide（人間）
- **裁定日**: 2026-07-29
- **設計移行**: 許可
