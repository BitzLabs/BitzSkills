...
target: audit の偽装耐性・解除区分の実効性・cause 語彙の schema 反映・設計文書の自己矛盾
proposed_change_type: modify
status: open
---
- **目的**: `SI-FLW-067` の対処（PR #294）で**閉じなかった中核**を閉じる。
  M2 出口条件6と4は今回も未達である。

- **発見した事実**（独立レビュア複数が別経路で実測）:
  1. **chain 検証は破損を止めたが偽装を止めない**（`DIN-203` critical / `RSK-404`）—
...
- **提案する修正**:
  - receipt chain へ**真正性の錨**を与える（common-dir の owner-only 鍵による HMAC 等）。
    末尾切詰めを検出できるよう最終 sequence を chain 外へ記録する
