...
**状態**: **一部裁定（P1 3件・P2 4件裁定・反映済み、P3 1件は未裁定）**
...
実装根拠にしない。P1 3件とP2 4件の裁定結果と反映先は§7・§8を正とし、P3 1件は引き続き未裁定である。
...
| UC-FLOW-008 | P3 | ADRの部分改訂を`related`で表しつつ、READMEは`supersedes`を要求 | accepted ADR間の優先関係が人間依存 | 未裁定 |
...
P3 1件（UC-FLOW-008: ADR部分改訂の表現）が未裁定である。ADR-030がADR-018 Decision 2を部分的に
置き換えた際にNotesとRevision Historyで注記しており、この扱いを正式な規約とするか、統合後継ADRへ
移行するかを裁定する必要がある。裁定まで、§4.8の修正例を記録規約として扱わない。
