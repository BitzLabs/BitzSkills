...
**状態**: **一部裁定（P1 3件裁定・反映済み、P2 4件・P3 1件は未裁定）**
...
実装根拠にしない。P1 3件の裁定結果と反映先は§7を正とし、P2・P3は引き続き未裁定である。
...
| UC-FLOW-004 | P2 | 変更コード・テストから検査対象SPECへの逆引きが未定義 | コードだけの変更で通常`check`が空になる可能性 | 未裁定 |
| UC-FLOW-005 | P2 | Small FlowのIntent起点語彙が一致しない | リファクタリング等の開始方法が不定 | 未裁定 |
| UC-FLOW-006 | P2 | Full FlowにDoneとレビュー否決時の戻り先がない | 完了・再作業条件が不定 | 未裁定 |
| UC-FLOW-007 | P2 | 削除済みID再利用禁止の履歴範囲が未定義 | commitをまたぐ再利用を検出できない | 未裁定 |
...
P2 4件（UC-FLOW-004〜007）とP3 1件（UC-FLOW-008）は未裁定である。§5の推奨裁定順に従い、
UC-FLOW-004・005、UC-FLOW-006、UC-FLOW-007・008の順に個別へ裁定する。裁定まで、これらの修正例を
Core 1.0の機械契約にしない。
