# Core 1.0 実装前異常case reviewと修正提案
...
正常系と既存のGit縮退、空対象、終端状態、report書込み失敗、path逸脱は実装可能な粒度に達していた。
...
| EDGE-002 | P1 | Stopped時の途中成果物とphase rollback単位が不定 | 縮小採用・反映済み |
...
- Core検査を迂回した過去の削除後におけるID再利用禁止はGit reviewの責務とする。
...
## 3. EDGE-002: Stoppedとphase checkpoint

### 指摘

Implement後に取り止めた場合、途中のcode・testを残すか破棄するか、StoppedのGit記録へ混在させるかが
未定義だった。またflowを取り消しやすいGit commit境界がなかった。
...
- 変更を伴うphaseは個別commitへ分け、変更のないphaseは現在HEADをcheckpointとして空commitを作らない。
...
| CoreまたはSkillによる自動rollback | 利用者の未commit成果物を破壊する可能性がある |
| 全phaseの空commit | rollback可能性を増やさずGit履歴だけを増やす |
...
- 標準出力と標準エラー出力は終了までdrainし、それぞれ末尾64 KiBまで保持して、超過した古いbyte列を破棄する。
...
また複数file読取り中の同時編集を完全に排除する契約もなかった。
...
- 複数fileを横断する原子的snapshotと読取り後の変更検出はCore 1.0の保証外とし、編集と検査を
...
| 案 | 不採用理由 |
|---|---|
| filesystem全体の原子的snapshot | 小規模チーム向け読取り専用Coreとして実装負荷が大きい |
| 全入力の前後hash再検査 | verify command自身の副作用と競合し、再実行分岐が増える |
| 不正UTF-8の置換継続 | 行・ID・hashが原文と異なる状態で成功し得る |

## 6. 反映先

- EDGE-001: ADR-037、EARS-AI規格/01・06、SPEC file規定/01・06・07・12
- EDGE-002: SDD process設計、運用設計、ユースケース設計、ADR-036 Notes
- EDGE-003: 共通アーキテクチャ、セキュリティ設計、SPEC file規定/06、ADR-026・030 Notes
- EDGE-004: SPEC file規定/06・07・11
