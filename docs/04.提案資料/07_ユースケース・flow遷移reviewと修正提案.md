...
実装根拠にしない。8件の裁定結果と反映先は§7〜§9を正とする。
...
`02.設計書/09_ユースケース設計.md`、`03.詳細設計/02_SPECファイル規定/05_補助SPEC仕様.md`、
`03.詳細設計/02_SPECファイル規定/06_参照・トレース・検証仕様.md`、
`03.詳細設計/02_SPECファイル規定/10_Context Resolution仕様.md`
...
§4.3の提案を採用し、[ADR-030](../02.設計書/10_決定記録/ADR-030_verify実行bindingの正規識別子と重複排除単位の統一.md)
として裁定した。binding正規識別子を`(workspaceId, 正規化argv template, 正規化cwd)`とし、command名を
識別子から除外した。`{tests}`の有無にかかわらずbindingを1回だけ実行し、単一workspace、引数なしverify、
...
反映先: `03.詳細設計/02_SPECファイル規定/02_bitz.yaml仕様.md`、
`03.詳細設計/02_SPECファイル規定/06_参照・トレース・検証仕様.md`、
`03.詳細設計/02_SPECファイル規定/12_モノレポSPEC連合仕様.md`、
`02.設計書/08_実装ロードマップ.md`、ADR-018・ADR-026のNotes

### 7.4 P1時点の残件

P1裁定時点で、P2 4件（UC-FLOW-004〜007）とP3 1件（UC-FLOW-008）が残った。P2 4件は§8で裁定した。
...
REQ/TECHへの逆参照だけを対象としており、実装変更を起点にしていないためである。

反映先: `02.設計書/01_共通アーキテクチャ.md`、`02.設計書/02_specディレクトリ仕様.md`、
`02.設計書/05_QA品質保証設計.md`、`02.設計書/08_実装ロードマップ.md`、
`03.詳細設計/02_SPECファイル規定/06_参照・トレース・検証仕様.md`
...
反映先: `02.設計書/04_SDDプロセス設計.md` §3、`02.設計書/09_ユースケース設計.md` UC-07、
ADR-028 Decision 6への追記
...
`03.詳細設計/02_SPECファイル規定/01_配置・命名・探索規則.md`、
`03.詳細設計/02_SPECファイル規定/04_要求SPEC仕様.md`、
`03.詳細設計/02_SPECファイル規定/07_更新・互換性・安全性.md`、
`02.設計書/08_実装ロードマップ.md`、`04.提案資料/README.md` §4

### 8.5 P2時点の残件

P2裁定時点で、P3 1件（UC-FLOW-008: ADR部分改訂の表現）が残った。§9で裁定した。

## 9. P3裁定結果（2026-08-31）

### 9.1 UC-FLOW-008

§4.8の第1案（`x-amends`）と第2案（統合後継ADR）はいずれも採らず、現行運用を正式な規約として確定し、
[ADR-033](../02.設計書/10_決定記録/ADR-033_部分改訂ADRの記録規約.md)として裁定した。

裁定にあたり、正本の現状を調べた。`accepted`のまま部分改訂されたADRは5件（ADR-005、009、010、014、018）
あり、いずれも旧ADRの`Revision History`がReference列へ後継IDを持つ。運用としては既に一貫しており、
欠けているのは、どのDecision項目が置き換わったかの明示だけであった。ADR-026はADR-030から参照されるが
Decision項目は置き換わっておらず、部分改訂ではなく明確化である。

裁定内容は次のとおり。

1. 部分改訂を許可し、旧ADRを`accepted`のままとする。全Decision項目が置き換わる場合だけ
   `supersedes`／`superseded`を使う。
2. 部分改訂は3点で記録する。後継ADRのDecision本文への明記と`related`、旧ADRの`Notes`、
   旧ADRの`Revision History`（Summaryへ対象Decision項目、Reference列へ後継ADR ID）。
3. Decisionが番号付きでないADRでは、該当箇所を一意に特定できる語句を用いる。番号を後付けするために
   Decision本文を書き換えない。
...
統合後継ADRを採らなかった理由は、ADR-009とADR-010の現行決定の大半が有効なまま設計書本文から
多数参照されており、`superseded`とすることが事実に反するためである。ADRを追記型の記録とする
決定記録README §1の前提とも矛盾する。

ADR-033自身がADR-020 Decision 4の部分改訂であり、本規約の最初の適用事例となる。

反映先: `02.設計書/10_決定記録/README.md` §2、`ADR-005`、`ADR-009`、`ADR-010`、`ADR-014`、`ADR-020`の
`Notes`と`Revision History`
