...
- 目的: 適合fixture `SINGLE-104-01`（`--format`省略のcontextがstdoutと期待Markdownでbyte一致）を
...
2. 表示順が10セクションであること（context仕様§9）
3. adapter命令をBundle外から与え、本文をsystem instructionへ昇格しないこと（同§9）

このため、見出しの記法、各セクションの中身、文書projectionごとの表示、空セクションの扱い、
...
- **セクションを省略しない。** 10セクションの見出しを常に同じ順で出し、該当が無ければ固定の1行を置く。
...
2. 文書titleは`# Context Bundle`固定のH1 1個とする。10セクションはH2、文書とstatementの明細はH3とする。
...
文書は結果JSONの`documents[]`順のまま各セクションへ配り、セクション内で再sortしない。
...
| P0-2 | `role: constraint`の配置 | (a) Required Contextへ併合 (b) 独立セクションを追加 | (a)。§9の10セクションを増やさない |
| P0-3 | `normative`／`reference`の本文 | (a) 本文を出さず`expandable`と参照だけ (b) 要約を生成 | (a)。Coreは要約を生成しない |
| P1-1 | `bodyText`への制御文字escape | (a) 原文のまま (b) text出力と同じ`\u00xx`可視化 | (a)。原文改変を避ける。ただしterminal安全性はadapterの責務と明記する |
| P1-2 | `compact` detailの文書セクション | (a) 見出しと`- <id>: <path>`だけ (b) 文書セクション自体を省略 | (a)。セクション不省略の原則を保つ |
| P1-3 | 連合結果の表示 | (a) `workspaceId`を見出しへ`<ws>::<id>`で出す (b) 一覧fieldへ出す | (a)。修飾IDの正規形と一致させる |
...
P1-2は`compact`でも見出しを残す。P1-3は連合の修飾IDをそのまま見出しへ使う。P1-4は`durationMs`を提示しない。

## 5. 完全例

入力は`SINGLE-042`のcorpus（`REQ-001`と`TECH-001`）、invocationは
`bitz context REQ-001 --purpose verify`、detailは既定の`standard`である。
§3の規則と§4の推奨を適用すると、stdoutは次のbyte列になる（`<64桁>`は実際のDigest）。
