...
| [08_フロー終端・遷移条件レビューと修正提案.md](08_フロー終端・遷移条件レビューと修正提案.md) | 02.設計書、03.詳細設計 | **Open**（5件裁定済み、3件未裁定） |

## 3. 検討結果の要約

レビュー01とAntigravityレビューは2026-08-27時点で全提案を裁定した。
以下の§3〜§7はレビュー01・02を閉じた時点の記述である。レビュー03と後続レビューは§8〜§13を正とする。
レビュー01〜07は全提案を裁定済みで、レビュー08は4件を裁定済み、現在の未裁定事項は3件である。
...
## 13. フロー終端・遷移条件レビュー（2026-08-31、5件裁定済み）
...
|---|---:|---:|---|
| P1 | 2 | 0 | — |
| P2 | 3 | 1 | Small FlowのHuman Review否決 |
| P3 | 0 | 2 | ADR部分改訂規約の適用範囲、明示対象と正規化対象の差 |
...
残る3件は未裁定であり、それらの提案内の修正例をCore 1.0の実装根拠にしない。裁定済み5件は
ADR-034、ADR-035、ADR-036と反映後の正本を実装根拠とする。
