...
| [08_フロー終端・遷移条件レビューと修正提案.md](08_フロー終端・遷移条件レビューと修正提案.md) | 02.設計書、03.詳細設計 | **Open**（7件未裁定） |

## 3. 検討結果の要約

レビュー01とAntigravityレビューは2026-08-27時点で全提案を裁定した。
以下の§3〜§7はレビュー01・02を閉じた時点の記述である。レビュー03と後続レビューは§8〜§13を正とする。
レビュー01〜07は全提案を裁定済みで、現在の未裁定事項はレビュー08の7件である。
...
## 13. フロー終端・遷移条件レビュー（2026-08-31、未裁定）

レビュー07の8件を裁定・反映した状態で、開発フローとSPEC状態機械を終端と遷移条件の観点から再確認した。

| 優先度 | 件数 | 主な内容 |
|---|---:|---|
| P1 | 2 | TASKの`open -> done`遷移のフロー配置、引数なし`check`の対象0件時status |
| P2 | 3 | Small FlowのHuman Review否決、`done` TASK起点の可否、check通過条件とwarning |
| P3 | 2 | ADR部分改訂規約の適用範囲、明示対象と正規化対象の差 |

P1 2件は、ADR-029が`requires`先TASKの`done`を実行可能性のゲートにしたことと、ADR-031が検査対象集合の
空を正常経路にしたことによって顕在化した欠落である。

指摘、修正候補、反映先、受入条件は
[08_フロー終端・遷移条件レビューと修正提案.md](08_フロー終端・遷移条件レビューと修正提案.md)を正とする。
現在は未裁定であり、提案内の修正例をCore 1.0の実装根拠にしない。
