...
| [10_並行開発におけるID重複対策の検討と提案.md](10_並行開発におけるID重複対策の検討と提案.md) | 02.設計書、03.詳細設計 | **Open**（未裁定・採決事項9件、P1 3件は適用差分あり） |

## 3. 検討結果の要約

レビュー01とAntigravityレビューは2026-08-27時点で全提案を裁定した。
以下の§3〜§7はレビュー01・02を閉じた時点の記述である。レビュー03と後続レビューは§8〜§14を正とする。
レビュー01〜09は全提案を裁定・反映済みである。提案10は起票済みで未裁定であり、
現在の未裁定事項はこれだけである。
...
## 15. 並行開発におけるID重複対策（2026-09-01、未裁定）

並行branchで独立採番された文書IDの衝突は、現行設計では検出できるがmerge後にしか検出できない。
予防機構と早期検出機構がなく、正規ファイル名が`<ID>-<slug>.md`を許すためGitのadd/add conflictも
働かない。詳細と提案は
[10_並行開発におけるID重複対策の検討と提案.md](10_並行開発におけるID重複対策の検討と提案.md)を正とする。

提案の骨子は、統合先branchを勝者とする先勝ち規則と、PR側の自動振り直しである。`--base`へ統合先先端を
渡すことで、既存のGit基準版（ADR-025）と`(workspaceId, documentId)`索引（ADR-037）だけで勝敗が決まり、
新しい入力機構、永続台帳、Git履歴走査を追加しない。ADR-037が裁定した「削除と再作成の交差」に対する
保証範囲は変更しない。

| 論点 | 選択済みの方針 | 状態 |
|---|---|---|
| 振り直しの実行主体 | Skill／拡張へ委譲し、Coreは検出と機械可読な提示に限る | 未裁定 |
| 新IDの採番 | 基準版と現在集合の全IDの最大値+1 | 未裁定 |
| 対象種別 | REQ、TECH、TASK、ADR | 未裁定 |

ユースケース設計と状態遷移契約への接続にあたり、採決事項9件をIDDUP-001〜009として起票した。
うちP1は3件である。

| ID | 優先度 | 指摘 |
|---|:--:|---|
| IDDUP-001 | P1 | 振り直しが基準版との比較でSPEC削除と誤判定される |
| IDDUP-002 | P1 | 開発フローに統合段階がなく、衝突解決の位置を定義できない |
| IDDUP-003 | P1 | 振り直しを独立ユースケースとするか、UC-04へ収めるか |

IDDUP-001はADR-037 Decision 3がID変更をrenameとして表現できないことに起因し、振り直し直後の
`check`を通さない。実装を止める衝突であり、他に優先して裁定する。P2・P3の6件と修正候補は
提案10 §9を正とする。

P1 3件は正本へそのまま適用できる差分を提案10 附録Aへ用意した。振り直しを統合先との合流をGitへ記録した
後の木で行うことを正規手順とし、勝者が旧IDを保持するためキーが消えない。これによりADR-037のDecisionを
変更せず、Frontmatter Schemaへも項目を追加しない。追加規則は基準版が同一キーに複数pathを持つ場合の
`SPEC-BASE-AMBIGUOUS-001`／warningだけである。改番の記録は`Revision History`へ置く。

検討過程で`renumberedFrom`および`x-renumbered-from`を候補としたが撤回した。前者はGitから導ける情報を
Frontmatterへ持ち込み、ADR-015 Decision 4と旧SPEC知見の「成果物ごとのversion、updated」と同型の陳腐化を
生む。後者は`x-`拡張を合否判定へ使用しない規則に反する。不採用理由はADR-038骨子のAlternativesを正とする。

開発フロー骨格への`Integrate`追加とUC-13の新設を含め、ADR-038として1件にまとめて裁定する。
ADR-028とADR-037はADR-033の3点記録による部分改訂とし、いずれも置換しない。
