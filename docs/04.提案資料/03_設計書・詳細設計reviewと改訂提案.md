...
- P3裁定日: 2026-08-31（新規ADRなし。既存決定を変更しないため本書§5と
  [04.提案資料/README.md](README.md)を記録先とする）
...
本節の5件は**すべて採用し、正本へ反映済み**である。既存の決定を変更しないため新規ADRは起こさず、
裁定は本書§5と[04.提案資料/README.md](README.md)を記録先とする（[附録C.0](#c0-全体方針)）。
適用した差分は[附録C](#附録c-p3修正案適用可能な差分)に残す。以下の「現状」は適用前の記述である。
...
```markdown
- 他Profileの語彙を自Profileの検証規則の前提にしない。例外は設けない。Profileは所有プラグインが提供するため、Profile間の依存は拡張プラグイン間の必須依存と同義になり、[ADR-016](../../02.設計書/10_決定記録/ADR-016_Agent-Plugins準拠の複数plugin配布.md)が禁止した構成になる。
...
`02.設計書/10_決定記録/ADR-018_正本Schemaの欠落補完とDiagnostic-severityの明示.md` として新規作成する。
`10_決定記録/README.md` §3の一覧へ次の行を追加する。

```markdown
| [ADR-018](ADR-018_正本Schemaの欠落補完とDiagnostic-severityの明示.md) | 正本Schemaの欠落補完と診断severityの明示 | Accepted | EARS-AI規格/01・02・06, SPECファイル規定/02〜04・06・11・12 |
```
...
- 未知拡張は `EAI-EXT-UNKNOWN-001` ／ warning とする。Core 1.0に厳格モードを設けず、
  警告種別ごとの終了コード昇格も行わない（[レビュー01 §6.1裁定](../../04.提案資料/01_詳細設計reviewと改訂提案.md)）。
```
...
[ADR-016](../02.設計書/10_決定記録/ADR-016_Agent-Plugins準拠の複数plugin配布.md) §4の
...
作成日: 2026-08-31。§4の5件について、正本へそのまま適用できる修正を示す。
行番号は附録B適用後の状態を指す。
...
5件はいずれも既存の決定を変更せず、既に決まっていることを読み取れる形にするだけである。
[決定記録](../02.設計書/10_決定記録/README.md)は「設計上の重要な決定とその理由」を保持する場であり、
表記の統一と記載漏れの補完まで記録すると、実際の判断がADR一覧の中で埋もれる。
...
| 4.5 `CTX-COVERAGE-*`のseverity | なし。[Context Resolution仕様](../03.詳細設計/02_SPECファイル規定/10_Context%20Resolution仕様.md) §9の本文にある規則を表へ写すだけ |
...
附録B.5で`verify`を分離済みであり、`check`だけが結合形のまま残っている。

#### 変更1: `02.設計書/03_CLI統合設計.md` §3（`:36-37`）
...
`--all-workspaces`が`--full`を含意し対象ID・pathを受け付けないこと、
`--workspace`と排他であることを文法上でも表す。

`doctor`は対象ID・pathを取らないため、`[--workspace <id>|--all-workspaces]`の結合形のままでよい。

#### 適合性fixture

- `bitz check REQ-001 --all-workspaces`が引数不正（終了コード4）になる
- `bitz check --workspace web --all-workspaces`が引数不正になる

### C.2 修正案⑭ — `Revision History`の行を採用側の表へ移す（§4.2）

#### 変更1: `03.詳細設計/02_SPECファイル規定/09_旧SPEC知見の評価.md` §4
...
§4に残る「成果物ごとのversion、updated」は正しく不採用であり、変更しない。
移動後は、隣接する2行が「不採用のversion/updated」と「簡素化して採用したRevision History」に
正しく分かれる。

### C.3 修正案⑮ — hooksの適用範囲を明示する（§4.3）

#### 変更1: `02.設計書/07_セキュリティとガバナンス.md` §6（`:73`）

- **before**

```markdown
- MCP serverとhooksはローカルコードを実行しうるため、公開前レビューと利用者への権限説明を必須とする。
```
...
供給網要件がhooksを対象に含めることと、Core 1.0がhooksを出荷しないことは両立する。
どちらか一方だけを読んだ実装者が「hooksを実装してよい」と誤読しないようにする。

### C.4 修正案⑯ — `check`の読取り専用性を1つの表現へ統一する（§4.4）

3文書が異なる範囲を指している。最も正確な
[更新・互換性・安全性](../03.詳細設計/02_SPECファイル規定/07_更新・互換性・安全性.md) §2の表現へ揃える。

| 文書 | 現行 | 成立するか |
|---|---|---|
| `07_更新・互換性・安全性:12` | 「`--report`を指定しない限りSPECを変更しない」 | 成立（正とする） |
| `SPECファイル規定/README:16` | 「読取り専用とし、成功時に生成物を残さない」 | 前半が不成立 |
| `07_セキュリティとガバナンス:32` | 「`check`は読取り専用とする」 | 不成立 |

#### 変更1: `03.詳細設計/02_SPECファイル規定/README.md` §2 原則4（`:16`）
...
```markdown
4. `bitz check`はSPECとコードを変更しない。書き出すのは`--report`指定時と失敗時の
   `.spec/reports/`だけとし、それ以外の生成物を残さない。
```

#### 変更2: `02.設計書/07_セキュリティとガバナンス.md` §3（`:32`）
...
`bitz context`は`--report`を持たないため、この整理の対象外である。
...
5件は相互に独立であり、任意の順で適用できる。検証は次のとおり。

```text
grep -n "bitz check\|bitz verify" docs/02.設計書/03_CLI統合設計.md
grep -rn "最終H2の\`Revision History\`" docs/03.詳細設計/
grep -rn "check\`は読取り専用" docs/
grep -n "CTX-COVERAGE" "docs/03.詳細設計/02_SPECファイル規定/10_Context Resolution仕様.md"
```
...
1. 本書§5の表へP3 5件の裁定を記入する
2. 本書の状態を`Closed（全項目裁定済み）`とし、P3裁定日を追記する
3. [04.提案資料/README.md](README.md) §2の一覧を`Closed`にする
