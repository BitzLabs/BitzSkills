...
`workspace-id::local-id`を文書自身のIDへ混入させず、複合workspace解決envelopeとして扱う設計は妥当である。
非修飾参照をsource workspaceだけから解決し、横断参照へ修飾IDを必須にしたことで探索順依存も排除できている。

主な不足は、複合workspaceのContextの追加fieldとCanonical Digest入力が実装可能なSchemaまで閉じていないこと、および
...
| ID | 優先度 | 指摘 | 影響 |
|---|---|---|---|
| FED-CTX-001 | P1 | 複合workspaceのContext追加fieldの完全なSchemaがない | adapterと実装で必須field・null・順序が分岐する |
...
- `documents[].workspaceId`: 複合workspaceでは必須、単一では省略可または実効ID必須のどちらか
- `resolution.workspaces[]`: `{id, path}`、request workspaceを先頭、その後ID順
- `resolution.crossWorkspaceEdges[]`: `{relation, source, target}`、3field必須
- `relation`の列挙値と、source/targetが文書IDかstatement IDか
- 複合workspaceでedgeが0件のとき空配列を返すか省略するか
...
横断参照のworkspaceまたはtargetが不在の場合、複合workspace仕様の`SPEC-MONOREPO-REF-001`と、関係モデルの
