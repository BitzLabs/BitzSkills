...
matrix 311件中、導入・設定9件、EARS-AI構文・候補抽出・拡張12件、文書構造・UTF-8 9件、関係・path・coverage 6件、ID重複・循環4件、Git基準版5件は入力・manifest・完全期待JSON・read-only副作用期待値を作成し、
各2回の隔離setupを固定snapshotと照合した。実fixture残件は266件であり、Coreは未実行である。
