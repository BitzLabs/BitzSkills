...
| 7.3 purpose別coverage | 反映済み | `interpret`、`implement`、`verify`の扱いを表で定義 |
...
| 9.7 Digest不一致の能力表追記 | Core 1.0未採用 | `CTX-STALE-001`と共通status/終了コードを正本とし、能力不足と実行結果を同じ表へ混在させない |
| 11 Agent Plugins分割配布 | 反映済み | ADR-016、共通設計、CLI、運用、doctor、ロードマップへ反映 |

「1.1以降の候補」はCore 1.0では不採用として閉じた項目であり、自動的な実装予定を意味しない。
記載した再評価条件を満たした場合だけ、新しい提案またはADRとして起票する。
...
各項目は次の形式で記述した。本書の提案本文は裁定材料の履歴として残す。
...
決定理由は[ADR-016](../02.設計書/10_決定記録/ADR-016_Agent-Plugins準拠の複数plugin配布.md)へ記録し、
共通アーキテクチャ、CLI統合、運用、セキュリティ、ロードマップ、`doctor`詳細仕様を同時に改訂した。
