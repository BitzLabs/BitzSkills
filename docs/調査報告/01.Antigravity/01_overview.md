...
従来のAI開発支援と異なり、開発プロジェクトの中に独立した専門役割を持った複数の「Subagents（サブエージェント）」や、特定の作業手順・規約を学習した「Skills（スキル）」を埋め込み、これらを柔軟に協調動作させることが可能です。
...
1.  **Antigravity Desktop (Desktop IDE)**:
    VS Code をベースとしてフォーク・カスタマイズされた統合開発環境（IDE）です。視覚的なエージェントのオーケストレーション、プロジェクト構造の把握、デバッグ、エージェントとの対話をGUIから行えます。UI は、複数エージェントのタスクを俯瞰・指揮する **Manager view** と、コードを直接編集する **Editor view** の2つのビューを中核とします。
...
> [!NOTE]
> 2026年時点の Antigravity は **Gemini 3 系を主軸**に、Claude Sonnet / Opus・GPT-OSS 等を含む**マルチモデル対応**です。本書に残る `gemini-2.5-*` はいずれも旧世代の表記です。

*   **Artifacts（検証可能な成果物）**:
    エージェントは作業の過程で、タスクリスト・実装計画・ブラウザ操作の録画といった「**検証可能な成果物（Artifacts）**」を生成します。ユーザーはこれらを通じてエージェントの意図と実際の挙動を確認・レビューでき、エージェントファースト設計における透明性の中核をなします。
*   **ブラウザサブエージェント**:
    Web ドキュメントの参照や Web アプリのレンダリング結果の検証を行う専用の**ブラウザサブエージェント**を備え、動的なページ確認や E2E 的な検証を自律実行できます。
