...
*   **`model`** (string): デフォルトで使用する LLM モデル。2026年時点では Gemini 3 系を主軸に、Claude Sonnet/Opus・GPT-OSS 等を含むマルチモデルから選択します（旧世代の `"gemini-2.5-*"` 表記は古い）。
*   **`enableTerminalSandbox`** (boolean): エージェントが実行するシェルコマンドを安全なサンドボックス内に隔離するかどうか。
    > [!NOTE]
    > 既定値はワークスペース資料では `true`、外部情報では `false` とする記述もあり、`agy` のバージョンで異なる可能性があります（要確認）。挙動を前提にする場合は明示的に値を設定してください。
*   **`toolPermission`** (string): ツール実行時の自律権限を設定します。ワークスペース資料では次の**4段階**とされます（要確認）。
    *   `"request-review"`（既定）: コマンドや重要ファイルの編集を実行する前に必ずユーザーに確認を求めます。
    *   `"proceed-in-sandbox"`: サンドボックス内で完結する操作は確認なしで進め、外部影響のある操作のみ確認します。
    *   `"strict"`: `request-review` より厳格に、幅広い操作で確認を要求します。
