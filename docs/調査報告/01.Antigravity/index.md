...
> [!NOTE]
> Antigravity は 2025年11月に Gemini 3 と同時発表された実在の製品です。骨格（IDE / CLI `agy` / SDK、`~/.gemini/config/`、`plugin.json`・`hooks.json`・`mcp_config.json` 等）は実態と一致しますが、Claude Code 互換レイヤーの詳細はワークスペース内の「実測」資料に依拠しており `agy` のバージョンで変わりうる点に留意してください。第6章末尾に既存章の正誤・補足を集約しています。
...
*   customization root 探索 / 5段階の名前解決 / フック内部契約 / 名前空間化 / 互換レイヤー内部 / **正誤・補足一覧**
