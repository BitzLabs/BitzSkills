...
OpenAI Codex CLI（`codex` コマンド、パッケージ名 `@openai/codex`）および SDK は、開発者のローカル環境でソフトウェア開発タスクを自律的に進めるために設計された、次世代の**AI エージェント型開発プラットフォーム**です。新しい Codex CLI は単なる補完ツールではなく、ファイルの探索・編集・テスト実行・ツール呼び出し・必要に応じた承認取得までを自律的に扱う実行エージェントとして位置付けられます。
...
3. **サンドボックスとセキュリティ**
   - ローカル環境のファイルやシステムを不要な破損から保護するため、OSレベルのサンドボックス（macOS の Seatbelt、Linux/WSL2 の Bubblewrap＋seccomp、Windows の専用ローカルユーザーアカウント方式など。詳細は第4章参照）を内蔵し、エージェントの行動範囲を物理的に制限します。
