...
### 1. 組み込みツールとカスタムツール
* **組み込み (Built-in)**: 
  * `bash`: シェルコマンドを実行するツール。後述の強力なOSレベルのサンドボックス環境下で実行されます。
  * `view_file`: 指定されたファイルを読み込んで閲覧します。
  * `write_to_file`: 新しいファイルを作成するか、既存のファイルを完全に上書きして書き込みます。
  * `replace_file_content`: ファイル内の特定の部分（ターゲットコンテンツ）を指定し、そこだけをピンポイントで置換・編集します。
