...
- **`verbose`**: 詳細ログの出力。
- **`telemetry`**: 利用統計の送信有無。
- **`maxThinkingTokens`**: Claude が推論に使用できる最大トークン数。
...
# 3. フィールドのパース
ENABLED=$(echo "$FRONTMATTER" | grep '^enabled:' | sed 's/enabled: *//' | sed 's/^"\(.*\)"$//')
STRICT_MODE=$(echo "$FRONTMATTER" | grep '^strict_mode:' | sed 's/strict_mode: *//' | sed 's/^"\(.*\)"$//')
