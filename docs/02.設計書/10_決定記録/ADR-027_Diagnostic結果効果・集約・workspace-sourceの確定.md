...
その診断が操作へ与える`resultStatus`がなく、条件によって複数statusへ対応するcodeを機械的に判別できなかった。
集約順序はverifyとdoctorに重複し、複合workspace結果ではworkspace相対`source.path`だけでは同名pathを一意にできなかった。

## Decision

1. Diagnosticの必須fieldへ`resultStatus`を追加する。値は`passed`、`passed_with_warnings`、
...
4. `source.kind: file`へ`workspaceId`を必須追加する。単一workspaceでも実効IDを記録し、
   `path`はそのworkspaceのroot相対とする。複合workspace root由来はroot workspaceのworkspace IDを使う。
...
- Diagnostic code表を知らないクライアントも、各診断の操作効果を構造的に読める。
- 全操作と複合workspace集約が同じ優先順位を使用する。
- 同じ相対pathを持つ複数workspaceの診断を一意に特定できる。
- Diagnostic fixtureとJSON例へ`resultStatus`、file sourceへ`workspaceId`を追加する必要がある。

## Alternatives

1. **code表からstatusを逆引きする**: 条件付きstatusのcodeでインスタンスを判別できないため採用しない。
