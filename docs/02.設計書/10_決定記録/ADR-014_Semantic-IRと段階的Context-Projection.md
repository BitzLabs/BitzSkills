...
3. **全ASTと全原文を常時渡す**: 情報が重複し、token消費とContext希釈が増える。
4. **依存深度で探索を打ち切る**: 深い位置の必須制約を欠いた部分Contextを正常に見せる。
...
| 対象 | 後継ADR | 内容 |
|---|---|---|
| Decision 6、7 | [ADR-015](ADR-015_SPEC改訂履歴の必須化.md) | 改訂履歴を非規範metadataとして意味集合から除外し、明示展開時の返却規則と`semanticHash`／`fileHash`の分離を定義 |
