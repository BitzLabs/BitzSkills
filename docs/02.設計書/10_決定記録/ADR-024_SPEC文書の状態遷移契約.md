...
状態遷移をadapterや自然言語スキルへ委ねると、同じ変更が実装経路によって合格または不合格になる。
...
7. Coreは影響候補を自動的に`outdated`へ変更しない。状態変更は人間が確認可能な差分として行う。
...
1. **状態値だけを検査する**: 禁止遷移をadapterごとに判断する状態が残るため採用しない。
...
- 本ADRは2026-08-31のP2残存契約review「状態遷移の不一致」に対する裁定である。
- TASKの`open -> done`を開発flowの終端へ配置する手順と、`done` TASKのpurpose別起点可否は
