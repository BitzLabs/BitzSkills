...
checkがこの規則を流用すると、検査対象SPECがない通常のcode変更までCIを停止する。checkとverifyでは
対象0件の意味が異なる。

またADR-028は「Post-checkが非成功のままVerifyへ進まない」と定め、ADR-034はTASK完了時checkが
「flowの通過条件」を満たしてからGitへ記録すると定めたが、`passed_with_warnings`を成功側へ含めるかを
...
4. check結果へ、`scope: changed`で使用する次のcheck固有fieldを追加する。
...
5. 引数なし`verify`の対象0件は従来どおり`SPEC-VERIFY-BLOCKED-002`／`blocked`とし、契約を変更しない。
   ...
   ことを表す。
6. Pre-check、Post-check、ADR-034のTASK完了時checkのflow通過statusを`passed`または
...
7. `failed`、`blocked`、`error`を非成功とし、次段階へ進まない。引数不正の終了コード4は操作statusを
   生成しないが、同じく次段階へ進まない。
8. warningはDiagnostic、text／JSON出力、明示reportへ保持するが、Core 1.0のflowでは自動昇格しない。
   Core 1.0へ`--strict`を追加しない。人間reviewまたはCore外のCI policyがwarningを理由に停止することは
   妨げない。
9. 本決定はADR-028 Decision 4の「非成功」とADR-034 Decision 1の「flowの通過条件」を明確化し、
...
- warningを残したままSmall/Full Flowを継続でき、終了コードとflow判定が一致する。
...
4. **`passed_with_warnings`をflow非成功にする**: 終了コード0との意味が分かれ、通常発生するwarningでflowが完了しないため採用しない。
5. **`--strict`でwarningを昇格する**: Core 1.0の共通終了コードを操作別policyで変えるため採用しない。

## Notes

- 本ADRは2026-09-01のflow終端・遷移条件review UC-FLOW-010およびUC-FLOW-013に対する裁定である。
- 関連文書: [01_共通アーキテクチャ](../01_システム構成.md),
  [04_SDD process設計](../03_SDD-flow.md),
...
|---|---|---|
| 2026-09-01 | check空対象とflow通過statusを確定 | `UC-FLOW-010`, `UC-FLOW-013` |
