...
PRのmerge-baseのどれを指すか、未追跡fileを含むかが未定義だった。また`bitz check`の公開文法は
...
5. 明示`check`はREQ、TECH、ADR、TASKの文書ID、規範文ID、それらのSPEC file pathを受け付ける。
   規範文IDとpathは所有文書IDへ正規化する。
6. code path、test path、directory、構文上不正なID／pathは引数不正として終了コード4を返す。
...
3. **code・test pathからSPECを逆引きする**: 多対多の暗黙選択となり、未対応pathの扱いも不安定なため採用しない。

## Notes

- 本ADRは2026-08-31のP2残存契約review「Git基準版」と「check明示対象」に対する裁定である。
- Decision 8の明示対象限定は、引数なし、`--full`、`--all-workspaces`でTASKが検査対象になった場合にも維持する。
  これらはTASK文書を検査するが`changes`境界を適用しない。完了・取止め時はTASK IDを明示して境界を検査する。
