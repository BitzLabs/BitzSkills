...
ADR-041とADR-042により、target別verify証跡、明示report保存、workspace同一性、全体事前検査、
canonical所有境界、公開結果Schema、resource上限、通常性能fixtureを確定した。一方、Core 1.0受入までに、
横断参照失敗のDiagnostic優先順位、workspace非成功後の継続範囲、TASK directory接頭辞とsymlink、
...
strong relation閉包からContextやverify planを作る。継続可否は全体事前検査と、文書・target・bindingの
依存単位に分けなければならない。

TASK境界では、symlink解決後の実pathを許可接頭辞との比較に使うと、字句上は列挙していないpathへ許可が広がる。
...
2. ADR-042の全体事前検査非成功だけを全停止境界とし、member処理、Context解決、command実行を開始しない。
   事前検査通過後は、checkは文書とsource edge、context／verifyはtargetのstrong relation閉包、verify commandは
...
4. TASK `changes`のfileとdirectory接頭辞は、正規化した字句Git pathの完全一致またはpath segment接頭辞で
...
5. JSON consumerは`schemaVersion` majorを確認した後、`workspace`だけを持つ単独結果と、`federation`および
   ...
   複合workspaceのproducerより先にconsumerをdual-read化し、adapter／CIは`monorepo.v1`を確認してから全体操作を有効にする。
6. rollbackはcatalog、member設定、修飾関係・coverage・TASK参照、CI全体操作、Capability gate、report consumerを
   同じrelease単位で戻す。過去reportを書き換えず、単一と複合workspaceの成果物系列を混ぜない。後続変更を失わずに
   全体を戻せない場合は自動down migrationを行わず、複合workspace状態をforward fixする。
...
9. 同名local ID、横断関係、Diagnostic優先順位、未登録設定、symlink、TASK接頭辞、継続、multi-context verify、
   共有binding、0件、同一性、Git不在、resource境界、report副作用、consumer、rollbackをversion管理fixtureと
   期待JSONへ固定する。`durationMs`等の指定済み非決定値だけを共通normalizerで除外し、fixture固有除外を許さない。
10. 本決定はADR-042でP2として残した契約を補完し、同ADRの同一性、Schema、resource数値、性能SLOを変更しない。
...
- TASKのdirectory接頭辞がsymlinkの解決先へ暗黙拡張されず、base側だけに存在するpathも検査できる。
- consumerの更新順、結果形式の判別、rollback可能条件が明示され、単一結果と複合workspaceの結果を誤集約しない。
...
3. **TASK接頭辞をsymlink解決後の実pathへ展開する**: 宣言していない字句pathまで許可集合へ入る。
4. **複合workspaceの結果を修飾IDの有無で判別する**: 空結果やDiagnosticだけの結果を安定して識別できない。
...
- 検討経緯とP2対応表は[提案23](../../04.提案資料/23_複合workspace残存P2裁定案.md)に記録する。
...
|---|---|---|
| 2026-09-03 | 複合workspace残存P2の継続、TASK境界、rollback、計算量、適合契約を確定 | FED-CTX-003ほか5件 |
