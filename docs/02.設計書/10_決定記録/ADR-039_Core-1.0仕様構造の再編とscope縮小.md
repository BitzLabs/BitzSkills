...
reviewで契約の欠落を解消する過程で、複合workspace、ID衝突の自動改番支援、将来Profile、4種類の公開hash、
必須改訂履歴、厳格な本文style、内容同一性によるverify binding重複排除が追加された。
...
1. 規範の所有者を共通契約、EARS-AI言語、SPEC文書モデル、Context解決モデル、操作仕様、SDD flowへ分ける。
...
5. Core 1.0は単一Git repository内の単一workspaceを対象とする。`monorepo.members`、修飾ID、横断所有、
   `--all-workspaces`、複合workspaceCapabilityを1.1以降へ延期する。結果のworkspace IDは`root`に固定する。
...
8. SPEC本文の`Revision History`をCore必須項目から外す。正確な差分、変更者、時刻はGitを正とし、重要な理由は
   ADR、PR、commit、または任意の`Notes`へ記録する。ADR directoryのローカルな記載規則は維持してよい。
...
11. Coreの本文構造検査はFrontmatter、H1、EARS-AI配置、REQの`Intent`、`Acceptance Criteria`、
    `Verification`へ限定する。H2順序、空の任意節、太字疑似節はtemplate・任意linterの責務とする。
...
- 仕様の変更理由が異なる機能は別文書となり、操作固有の変更が他操作の再reviewを要求しにくくなる。
- monorepo利用者はCore 1.0ではGit repository rootの単一`.spec/`を使用する。複数`.spec/`の複合workspaceはできない。
...
3. **公開操作を`context`と`check`だけに縮小する**: 実行testまでの垂直価値を検証できないため採用しない。
4. **複合workspaceとID改番だけ維持する**: 両機能は互いにworkspace修飾、基準版、集約Schemaへ波及するため、
...
- Decision 5の複合workspace延期、対応するConsequences、および本項の再導入条件は
  [ADR-040](ADR-040_複合workspaceをCore-1.0へ再導入する.md)で置き換えられた。Decision 1〜4、6〜13と、
  複合workspace以外のscope縮小は変更しない。
...
| 2026-09-02 | Decision 5の複合workspace延期を部分改訂 | ADR-040 |
