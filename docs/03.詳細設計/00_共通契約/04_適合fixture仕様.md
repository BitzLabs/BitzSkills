...
Gate Aの認定後にmatrixの行、またはfixtureの入力、期待値、副作用期待値を変更する場合は、変更を次の4種類に分け、
種類ごとの条件を満たす。判断の理由は
...
| 期待値の訂正 | 期待結果、期待text、副作用期待値、Parser期待値が規範文と食い違うもの | 規範文に合わせる。規範文自体を直す必要があれば先に規範文を変更し、Decisionに関わる場合はADRを起こす。人間の管理者の承認を要する |
| 削除・緩和 | 到達できない条件、規範文どうしの矛盾 | 規範文の側で裁定し、根拠を規範文へ注記する。人間の管理者の承認を要する |
| 非意味的な変更 | 名前、形式、検証基盤の整備で、入力と期待する動作が変わらないもの | 統合検証が通過する |

Core実装の観測出力は、期待値の訂正、削除、緩和の根拠にしない。Coreがfixtureを通過しないことは、
それだけではfixtureの欠陥を示さない。次のいずれかにあたる変更は緩和とする。

- 期待するstatus、終了コード、Diagnostic、report、副作用の制約を弱める
- 入力を変えて、fixtureが検査する条件が成立しなくなる
- normalizerまたは比較の範囲を広げる

変更の際は次を行う。

1. 追加、期待値の訂正、削除・緩和では、根拠の規範文と変更内容を該当fixture群のreviewへ日付付きの節で記録する。
2. fixtureの変更とCore実装の変更を同じcommitに含めない。
3. `uv run fixtures/validate_conformance.py`を通し、`uv run fixtures/certify_gate_a.py`でGate Aを再認定して、
   結果を適合fixture検証記録へ残す。
4. Diagnostic台帳の根拠文書が変わる場合は、台帳を再reviewする。
