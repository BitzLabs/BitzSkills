...
| 6 | 複合workspaceのcatalog、member、path所有 | failed／blocked |
| 7 | Schema major | blocked |
| 8 | EARS-AI major | blocked |
| 9 | Git利用可否と下限version | 単一はwarning、複合workspaceはblocked |
| 10 | command実行fileとcwd | blocked |
| 11 | 影響候補件数 | info |

独立検査は先行失敗後も可能な範囲で続行する。`--workspace`は選択member、`--all-workspaces`は同じGit rootと
root workspaceを探索起点から一意に発見できる場合にcatalogと全memberをworkspace ID順に診断する。current directoryの
root一致は要求しない。Core 1.0はProfile互換性を検査しない。
`--all-workspaces`ではCore、plugin、Capability、Git、catalogを最上位で1回検査し、workspace固有の設定、版、
command、cwd、影響候補を各member結果へ置く。同じ環境Diagnosticをmemberごとに複製しない。

全体診断はHEADと現在snapshotのGit既知`.spec/bitz.yaml`を、複合workspaceを宣言している各snapshot自身のcatalogと比較する。
unborn repositoryと複合workspace化前の単一workspace HEADでは現在snapshotだけを複合workspace完全性の対象にする。catalog、ID、path、
Git境界、未対応major、resource上限の全体事前検査が非成功ならmember診断を開始しない。

事前検査通過後はcheck項目を継続単位とする。先行checkの出力を必要としないCore、plugin、Capability、Gitは
...
複合workspaceではrepository境界と所有範囲を確定できないため`SPEC-MULTI-GIT-001`／blockedとする。
...
`--all-workspaces`結果は最上位に`core`とglobal `checks[]`を持ち、各workspace結果は0件でも省略しない
...
`apiVersion`をnull、`capabilities`を空配列にする。それ以外は観測した文字列値とCapability配列を返す。
...
複合workspace固有Diagnosticと全体結果外形は
