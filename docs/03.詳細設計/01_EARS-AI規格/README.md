...
これにより、人間は通常の文章として読み、Parserは同じ構造をASTへ変換し、AIは必要な要求だけを
選択して利用できる。EARS-AIはBitz AI-SDDの中心的な入力形式である。
...
Core 1.0の実装対象はCore Parser、AST、Validatorに限定する。Profile文書は拡張点を予約する設計資料であり、
Core 1.0の出荷条件ではない。
...
4. Markdownは1回だけ解析し、拡張は共通ASTを利用する。
...
| [06_AST・パーサー仕様.md](06_AST・パーサー仕様.md) | AST、Parser、Diagnostic | 必須 |
