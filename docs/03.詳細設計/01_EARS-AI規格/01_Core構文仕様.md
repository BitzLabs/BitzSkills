...
- IDは永続識別子とし、削除したIDの再利用を禁止する。Core 1.0はGit管理済みSPECの削除を状態遷移違反として
  防止し、現在集合内の重複を検出する。Core検査を迂回した過去の削除後における再利用禁止はGitレビューの責務とする
  （[ADR-037](../../02.設計書/10_決定記録/ADR-037_Git基準版間のSPEC同一性と削除規則.md)）。
