...
規範文には安定IDを付ける。IDは `<文書ID>:<ローカルID>` の**2階層固定**とする（[ADR-005](../../02.設計書/10_決定記録/ADR-005_規範文IDの階層と記法.md)）。見出し、背景、理由などの非規範文にはIDを強制しない。

```ebnf
document-id  = prefix, "-", 3DIGIT ;
prefix       = 2*8( UPPER ) ;                       (* REQ, TECH, DOMAIN, PLAN, TASK, ADR, RULE *)
...
4. 本文中にリテラルの `[` または `]` を書く場合、コードスパンで囲むか、`\[` `\]` とエスケープする。エスケープされていない `[` が既知タグとして解釈できない場合は `EAI-CORE-SYNTAX-004`（未閉鎖または不正なタグ）とする。
...
- 先頭の空白を除去した結果が `- [` で始まり、直後が `statement-id` に一致する行のみを規範文として解析する。
- フェンス付きコードブロック（``` および ~~~）の内部は解析しない。仕様書中の記述例が誤って規範文として解釈されることを防ぐ。
- 引用ブロック（`>`）の内部は解析しない。
- 上記に一致しない行は非規範文として扱い、IDや規範強度を要求しない。
...
同一規範文の自動翻訳同期はCore 1.0の対象外とする（[08_実装ロードマップ.md](../../02.設計書/08_実装ロードマップ.md) §8）。`text` の同一性判定は正規化後の文字列一致で行い、意味的同一性の判定を行わない。この判定は `outdated` 候補の生成に用いる。異なる言語の文書は警告としてよいが、構文解析自体を停止しない。
