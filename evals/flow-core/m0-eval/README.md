...
| **危険事象の proxy** | 4種それぞれの measurand・proxy・乖離条件を `FLW-DSN-014` が固定する。**乖離条件を書けない proxy は採用しない**（`SI-FLW-019` 案2） |
| **必須 field 保持の母集団** | **`flow.py` 呼出があった trial の中で**算出する。非呼出 trial は Invocation Rate 側だけに計上する（`SI-FLW-033`） |
| **測定不能** | `task_output_missing` に加え、**被測定物が一度も評価されていない trial**（`agent_unavailable`）も測定不能とし harness 再試行の対象にする（`SI-FLW-030`） |
...
### harness の自己診断（`SI-FLW-019` 案3。2026-08-11 裁定）

**被測定物の数値が良くても、自己診断が閾値を超えたら FAIL とする。**
測定系の欠陥9件はすべて数値が悪化して初めて発見されており、設計レビューで見つかったものは
1件も無い。すなわち従来は**合格が測定系の健全性の証明になっていなかった**。

| 自己診断メトリクス | 閾値 |
|---|---|
| 採点候補が2件以上あった trial の割合（platform × task） | **50%以下** |
| 採点対象が非 OK result だった trial の件数 | **0**（判定できない旧計装の記録は `0` と書かず「判定不能」を未達として出す） |
| 除外した呼出の件数と内訳（`--help` / 測定不能 / `tool_path_unknown`） | 記録して内訳を提示する |
| `instrumentation_gaps`（observation 共通部の欠落） | **0** |

#### 閾値 50% は実測から決めた（後知恵の緩和でないことの検証）

採点規則を変える以上「第12ラウンドを通すために緩めた」という批判は構造的に成り立つ。
`SI-FLW-019` の確認観点に従い、**再実測せずに過去ラウンドを採点し、欠陥が発見されるより
前のラウンドで検出できること**を機械的に示した。

| ラウンド | agy `diff-summary` の採点候補2件以上 | 当時の判定 | 自己診断 |
|---|---:|---|---|
| **r7**（2026-08-06） | **100%**（10/10） | — | **FAIL** |
| **r8**（2026-08-06） | **80%**（8/10） | **agy は全指標 pass** | **FAIL** |
| r10（2026-08-07） | 100%（10/10） | `SI-FLW-017` をここで発見 | FAIL |
| r12（2026-08-10） | 29%（6/21） | `SI-FLW-028` 是正後 | 指摘なし |

`SI-FLW-017`（採点対象の選択規則）が発見されたのは r10 である。自己診断は同じ欠陥を
**r7 / r8 で検出する**。とりわけ **r8 は agy が全指標 pass だったラウンド**であり、
`SI-FLW-019` 原因3 の実例そのものである。是正後の r12 では 3 platform とも指摘が出ないため、
「何でも FAIL にする検出器」ではない。健全なラウンドの `dirty-status` が常に 30〜40% なのは
`repo.inspect` → `git.status` の NEXT 連鎖で候補が2件になるためで、これが正常値である。

検証は `tests/test_m0_eval_scoring.py` がリポジトリ内の記録から実行する。

#### 新規則は第12ラウンドを合格させない

第12ラウンドの記録を新規則で再採点しても **`passed: false` のままである**。

- **必須 field 保持の未達は解消する**（`SI-FLW-033` の母集団是正が効く）
- **危険事象 2 件は残る。** `danger.state_change` / `danger.silent_truncation` は
  runner が trial 記録を作る時点で確定するため、proxy の是正（`031` / `032`）は
  **次のラウンドの実測にしか効かない**
- **計装の共通部の欠落**（`agent_unavailable` / `tool_path_unknown`）が 3 platform で出る。
  第12ラウンドは旧計装で測られた記録であり、新規則の下では出口判定の証跡にならない

すなわち本是正は、過去の判定を書き換えるものではない。
...
> **2026-08-11、`SI-FLW-019` を親として一括裁定し、測定系を是正した**
> （`plugins/bitz-flow/.spec/reports/decision-2026-08-11-si-flw-019-measurement-system.md`）。
> 未達5件のうち4件は `SI-FLW-019`（2026-08-07 起票）の未実施案の直撃であり、
> `030`/`033` は案5（3軸分解）、`031`/`032` は案2（proxy の乖離条件）に対応する。
> 是正は runner が trial 記録を作る時点の判定を変えるため、**効果は第13ラウンドの
> 実測にのみ現れる**（上記「新規則は第12ラウンドを合格させない」）。
...
> **2026-08-08 裁定（`decision-2026-08-08-si-flw-018-activation.md`）。** 本文が読まれる**前**に
> 分岐しているため、効き得るのは frontmatter の `description` だけである。落ちた trial の
> prompt「直前のコミットからの**変更量**を教えてください」は **`git` も `diff` も含まない**一方、
> 旧 description はコマンド名の列挙を主体に「何ができるか」を述べていた。同ラウンドの
> `repo-inspect` /`dirty-status` の prompt は description の語彙に近く、`diff-summary` だけが
> 遠かったことが、この task で落ちた説明として整合する。
>
> v2 fixture の description を「**git / gh を実行する前に必ず開く**」で始め、
> 「`git` / `gh` という語を含まない依頼でも発動する」と実例フレーズごと明記した（`0.5.0`）。
> **本文は 1 行も変えていない**（`FLW-DSN-010`）。prompt も据え置く — prompt に `git` を足すのは
> 被測定物ではなく問題の方を測定条件へ寄せることであり、`SI-FLW-009` の方針に反する。
>
> **0 件が出ても「直った」とは言い切れない。** `SI-FLW-026` の母数引き上げで claude の
> 1ラウンドあたり v2 母数は 30 → 60 になるが、達成できるのは 95% 上側信頼限界 **4.87%** までで、
> 元の実測レート **0.5%** を反証したことにはならない。より強い主張は Promotion Gate の
> 累積要件（299 trial 以上）へ繰り延べる。
