...
- **実施**: 2026-07-14 主提案（委譲ゲート）を CORE-FR-006 / CORE-FR-007 / CORE-FR-008 /
  CORE-FR-009 / CORE-CON-007 / CORE-NFR-001 と設計 DSN-001（委譲レジストリ）へ要件化し実装
  （CORE-TSK-007〜010）。PR #36 でマージ、当該要件はすべて verified 済み。派生として
  CORE-FR-010（spec_scaffold の生成時語彙検証。PR #39）と bitz-sdd の SDD-FR-112 を起票・実装。

---

## 裁定・確定設計（2026-07-13 チャット裁定）

**採否**: accept（open→accepted）。主提案（委譲ゲート）を採用し、下記の設計拡張を確定する。

**確定した設計 — 相対的・プラットフォーム別・機械検証付き委譲レジストリ**:

1. **相対選択（司令塔基準）**: 役割にモデルを絶対固定せず、司令塔（ユーザが起動したモデル）の
   階層内**相対位置**で判定する。
   - **下位への委託**: 自分より下位ティアがある時のみ委譲（無ければ自分で実行）。
   - **上位への相談・上申**: 自分より上位ティアがある時のみ（最上位なら相談相手なし＝自分で判断）。
   - **同一ティア経路の無効化**: 役割の束縛モデル == 司令塔モデル の経路は抑止
     （同一モデルへの委譲は省トークンにならない）。例: Opus が司令塔なら deep-reasoner(=Opus) は無効、
     fast-worker(Sonnet) へは委譲可。Sonnet が司令塔なら下位 worker は無く、難題は Opus へ上申。
   - antigravity(Gemini) は別プラットフォームで相対階層と**直交**（量産はいつでも可）。
   - **委託（下位）と相談・上申（上位）の両方を機構化する**（本裁定で確定）。
2. **プラットフォーム別レジストリ**: Claude と Antigravity はモデル体系が全く異なるため、
   レジストリを**分離管理**する（片方の更新が他方に干渉しない）。
3. **機械検証（分岐B採用）**: レジストリの整合を `release_check.py` / `spec_inspect.py` が検査する —
   委譲先 agent の実在、モデル名のレジストリ外直書き禁止、ティア順序の整合。
   これが「モデルが変わっても更新できる**仕組み**」の本体。

**プロセス**: 複数スキル横断＋契約（機械検査）に触れるため軽量レーン非適用。通常フロー＋Design Gate。
要件は後続で EARS 起票する。
