...
- **重複統合（2026-07-18, SI-CORE-027）**: 本ISSUEと完全に同一の問題（ルート単体 spec_inspect
  実行時の ENV-* 幽霊参照）が SI-CORE-023 として再起票され、その提案2（正規コマンドの明記）が
  実際に採用・実装された（AGENTS.md「仕様（.spec）検証の正規コマンド」節）。本ISSUE自身の裁定
  （上記）が未クローズのまま放置されていた。当初 `deprecated` へ遷移する方針だったが、
  spec-issue の権限マトリクスには `accepted → deprecated` 遷移が定義されておらず
  （`deprecated` は要件 (`requirement`) 専用の語彙で spec-issue には存在しない。`spec_update.py`
  が機械的に不正遷移として拒否）、`accepted` のまま留め置く方針に訂正した。
- **実施**: 2026-07-18（SI-CORE-027）SI-CORE-023 での実装をもって本ISSUEも解決済みとみなす
  （内容が完全一致する重複のため、実装は SI-CORE-023 側の記録を正とする）。
