...
| SINGLE-125-06 | SINGLE-072 | `check --full --base HEAD --format json --report` | error／3 | 読取り専用（保存失敗） |
...
監査は各fixtureを隔離repositoryへ2回setupし、固定snapshotと相互の一致を確認する。
回帰試験では、cacheへの書込み許容、HOMEの事前汚染、読取り専用caseへの`--report`追加とreport件数の改変、
追加環境変数、期待statusの改変、一時file残存の許容、report件数2件、explicit-reportから読取り専用への変更、
読取り専用caseへのreport directory追加、verify commandの書込みcommandへの置換をいずれも拒否する。
...
失敗check由来の元Diagnosticに`SPEC-REPORT-WRITE-001`が続く。副作用は読取り専用で、symlink自体、
