# approved REQの保護対象外変更fixture review
...
03は実在するTECHへの弱い関係とし、参照切れwarning・strong依存の影響候補を混ぜない。
...
既存git_fixtures.pyを拡張し、実入力と変更後byte列、固定YAMLとreview済みFrontmatter値のSchema、
manifest、完全期待JSON、読取り専用副作用期待値を照合する。汎用YAML解析や意味差分の判定は実装しない。
...
Coreの意味変更保護、副作用、実標準出力と終了コードはGate Bで受け入れる。
