...
| SINGLE-088 | titleがinteger | failed／1 | 0／0 |
...
line/column、evidence、suggestedActionはこの期待値では付加しない。診断summaryは期待JSONの文字列を固定する。
...
回帰試験は警告の削除、件数・診断コード・statusの改変、二重診断、副作用の許容と、各不正入力の修復を拒否する。
