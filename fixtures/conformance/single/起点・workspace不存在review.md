...
このcaseは単一起点であり、複数起点の部分不在の継続契約は対象外。

SINGLE-127-14は既存の有効なSINGLE-042 corpusを使う。`multiWorkspace`宣言がないためworkspace同一性はrootだけであり、
`missing`は空文字列でも不正な字句でもなく、探索後に不在と判定する。
...
回帰試験はworkspaceのrootへの置換、両caseの終了コード混同、失敗Contextのcomplete化とsource改変を拒否する。
