...
120-02だけがsetup operationで作業treeの同fileを更新する。stageもcommitもしない。auditはHEADとindexのblobがbase入力、
...
JSON Schemaの`uniqueItems`は配列順を区別するため、118-02を検出できない。auditはSchema違反またはkey tuple重複を
