...
### 並行開発（複数セッション・worktree が同時に動く前提）

このリポジトリは常に複数の作業が並行する。**メインの作業ツリーは共有されており、
他セッションの作業途中のファイルがそこに現れる**。`git worktree list` で稼働中の作業を
把握してから着手する。

- **ブランチは必ず `origin/main` から明示的に切る**: `git checkout -b <name> origin/main`。
  いま何がチェックアウトされているかに依存して切ると、他セッションのコミットを
  自分の PR に取り込んでしまう
- **`git add -A` と広いパス指定を使わない**: 自分が触れたファイルだけを個別に指定する。
  `git add -A -- plugins` のような指定は他セッションの未コミット変更を巻き込む
- **コミット直前に `git diff --cached --name-only` を確認する**: 自分の関心事以外の
  ファイルが1つでも混ざっていたら unstage する
- **事実は作業ツリーではなく確定した ref から読む**: ファイル数・要件数などを成果物へ
  記録するときは `git show origin/main:<path>` や clean な状態で測る。作業ツリーの
  `ls` / `grep` は他セッションの途中状態を含み、確定値として記録すると誤りになる
- 他セッションの作業中ファイル（`.spec/`・`docs/` を含む）は**読み取りのみ**とし、
  修正・移動・削除・コミットをしない
